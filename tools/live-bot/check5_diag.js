...
(async () => {
  ...
  while (!inShop && runNo < 4) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (!st.handStr) break;
      const d = decide(st.handStr); log('DECIDE', JSON.stringify(d), st.handStr);
      ...
      await sleep(2100);
      const st2 = await statusSnapshot(b1); log('AFTER', JSON.stringify({s:st2&&st2.score,h:st2&&st2.hands,d:st2&&st2.discards}));
