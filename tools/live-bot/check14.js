...
(async () => {
  ...
  const sampler = (async () => {
    while (Date.now() - t0 < 240000) {
      await sleep(40000);
      try {
        const out = await RCON('spark tps --timeout 150');
        const m = out.match(/(\d+\.\d+), (\d+\.\d+), (\d+\.\d+), (\d+\.\d+), (\d+\.\d+)/);
        if (m) { tpsSamples.push(m.slice(1).map(Number)); log('TPS', m[0]); }
  ...
  step('soak10_alive', bots.every(b => b.entity), `10/10 存活；${totalRuns} 局 ${totalHands} 手 / 4 分钟`);
  ...
  await sleep(1500);
  fs.writeFileSync(__dirname + '/check14-results.json', JSON.stringify({ results, stats, tpsSamples, memBefore, memAfter }, null, 2));
