...
(async () => {
  ...
  step('soak_10bots_no_kick', bots.every(b => b.entity), `4 bot 存活，共 ${totalRuns} 局 ${totalHands} 手`);
  ...
  monitor.quit();
  await sleep(1500);
  fs.writeFileSync(__dirname + '/check13-results.json', JSON.stringify({ results, stats, tpsReadings, memBefore, memAfter }, null, 2));
